...
resource manifest schema. The `path` property references a Python module metadata file
that encodes the module and class used for operation dispatch:
...
The `path` property (relative to the manifest file) references an adapter-specific metadata file
that contains the Python module and class information. This file is typically named `resource.json`
and contains:
...
- New venvs appear in `DSC_PYTHON_VIRTUAL_ENV_PATH` (resources need to be rediscovered from new locations)
