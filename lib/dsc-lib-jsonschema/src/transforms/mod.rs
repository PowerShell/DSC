...
pub fn idiomaticize_externally_tagged_enum(schema: &mut Schema) {
    ...
    let one_ofs = schema.get("oneOf")
        .expect("Applies only to tagged enums")
        .as_array()
        .expect("oneOf should always be an array");
    ...
    for item in one_ofs {
        let item_data: Map<String, Value> = item.as_object()
            .expect("items in 'oneOf' should always be objects")
            .clone();
        // If we're accidentally operating on an invalid schema, short-circuit.
        let item_data_type = item_data.get("type")
            .expect("every entry in oneOf should define the 'type' keyword")
            .as_str()
            .expect("the value for the 'type' keyword should be a string");
        assert!(
            !item_data_type.ne("object"),
            "invalid application of idiomaticize_externally_tagged_enum; expected type for oneOf entry to be 'object' but was {item_data_type}"
        );
        ...
        let properties_data = item_data.get("properties")
            .expect("each item in oneOf should define an object with properties")
            .as_object()
        ...
        for property_name in properties_data.keys() {
            ...
            let mut property_data = properties_data.get(property_name)
                .unwrap_or_else(|| panic!("the property '{property_name}' should exist in the object"))
                .as_object()
                .unwrap_or_else(|| panic!("the property '{property_name}' should always be an object"))
                .clone();
...
pub fn idiomaticize_string_enum(schema: &mut Schema) {
    ...
    // schema or subschema, it should fail fast.
    let one_ofs = schema.get("oneOf")
        .expect("Applies only to string enums")
        .as_array()
        .expect("oneOf should always be an array");
    ...
    for (index, item) in one_ofs.iter().enumerate() {
        let item_data = item.as_object().expect("msg").clone();
        // If we're accidentally operating on an invalid schema, short-circuit.
        assert!(
            !item_data.get("type").expect("subschema must define a type")
                .as_str().expect("'type' keyword should always be a string")
                .ne("string"),
            "idiomaticize_string_enum only applies to string enums"
        );
        ...
        if let Some(item_enum_value) = item_data.get("enum") {
            item_enum = item_enum_value.as_array()
                        .expect("enum keyword is always an array")
                        .index(0)
                        .as_str()
                        .expect("every item in the enum keyword array must be a string");
        } else {
            item_enum = item_data.get("const")
                        .expect("documented items in a string enum are generated as subschemas with the const keyword")
                        .as_str()
                        .expect("the value for the const keyword must be a string");
        }
