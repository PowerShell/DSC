...
use crate::vscode::keywords::{AllowCommentsKeyword, VSCodeKeywordDefinition};

#[test] fn meta_schema_is_valid() {
    let schema = AllowCommentsKeyword::default_schema();
...
#[test] fn boolean_value_is_valid() {
    let validator = keyword_validator!(AllowCommentsKeyword,  &json!({
        "allowComments": true
    }));
