...
use crate::vscode::keywords::{EnumSortTextsKeyword, VSCodeKeywordDefinition};

#[test] fn meta_schema_is_valid() {
    let schema = EnumSortTextsKeyword::default_schema();
...
#[test] fn non_array_value_is_invalid() {
    let validation_error = keyword_validator!(EnumSortTextsKeyword,  &json!({
        "enumSortTexts": true
    })).unwrap_err().to_owned();
...
#[test] fn string_array_value_is_valid() {
    let validator = keyword_validator!(EnumSortTextsKeyword,  &json!({
        "enumSortTexts": [
            "a",
            "b",
            "c"
