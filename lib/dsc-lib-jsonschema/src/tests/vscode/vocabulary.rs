...
#[cfg(test)] mod VSCODE_VOCABULARY_SCHEMA_BUNDLED {
    use crate::vscode::vocabulary::VSCODE_VOCABULARY_SCHEMA_BUNDLED;

    #[test] fn meta_schema_is_valid() {
        let schema = VSCODE_VOCABULARY_SCHEMA_BUNDLED.clone();
...
#[allow(non_snake_case)]
#[cfg(test)] mod VSCODE_VOCABULARY_SCHEMA_CANONICAL {
    use crate::vscode::vocabulary::VSCODE_VOCABULARY_SCHEMA_CANONICAL;

    #[test] fn meta_schema_is_valid() {
        let schema = VSCODE_VOCABULARY_SCHEMA_CANONICAL.clone();
        let result = jsonschema::meta::validate(
            schema.as_value()
        );
        assert!(result.is_ok(), "Unexpected error: {}", result.unwrap_err());
...
#[cfg(test)] mod VSCodeVocabulary {
    ...
    #[cfg(test)] mod json_schema_canonical {
        ...
        #[test] fn returns_schema_without_bundled_definitions() {
            let schema = VSCodeVocabulary::json_schema_canonical(
                &mut SchemaGenerator::new(SchemaSettings::draft2020_12())
            );
    ...
    #[cfg(test)] mod schema_resource_bundled {
        ...
        #[test] fn does_not_panic() {
            VSCodeVocabulary::schema_resource_bundled(
                &mut SchemaGenerator::new(SchemaSettings::draft2020_12())
            );
        }
    }
    #[cfg(test)] mod schema_resource_canonical {
        ...
        #[test] fn does_not_panic() {
            VSCodeVocabulary::schema_resource_canonical(
                &mut SchemaGenerator::new(SchemaSettings::draft2020_12())
            );
        }
    }
    #[cfg(test)] mod default_schema_resource {
        ...
        #[test] fn does_not_panic() {
            let _ = VSCodeVocabulary::default_schema_resource();
        }

        #[test] fn returns_resource_with_correct_id() {
            let resource = VSCodeVocabulary::default_schema_resource();
            let contents = resource.contents();
            let id = contents.get("$id").and_then(serde_json::Value::as_str);

            assert_eq!(id, Some(VSCodeVocabulary::SCHEMA_ID));
        }
