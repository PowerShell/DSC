...
///
/// // When a semantic version is compared to a date version, the semantic version is always
...
/// // if the string can be parsed as a semantic version. If the string can be parsed as a date
/// // version, the semantic version is always greater.
...
/// // identical, a stable date version sorts as higher than a prerelease date version. If the
/// // dates are identical and both versions are prerelease, the prerelease segments are compared
/// // lexicographically:
...
/// assert!(alpha < beta);
///
/// // Comparing a resource version to a string that doesn't parse as a valid resource version
