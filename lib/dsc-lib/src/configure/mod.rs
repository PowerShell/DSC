...
use crate::dscresources::{
    ...
    invoke_result::{DeleteResult, GetResult, SetResult, TestResult, ExportResult, ResourceSetResponse}},
...
impl Configurator {
    ...
    pub fn invoke_set(&mut self, skip_test: bool) -> Result<ConfigurationSetResult, DscError> {
        ...
        for resource in resources {
            ...
            let end_datetime;
            let mut set_result;
            let mut delete_result: Option<DeleteResult> = None;
            if exist || dsc_resource.capabilities.contains(&Capability::SetHandlesExist) {
                ...
                start_datetime = chrono::Local::now();
                // returns a DeleteResult in WhatIf mode
                delete_result = match dsc_resource.delete(&desired, &self.context.execution_type) {
                    Ok(result) => result,
                    Err(e) => {
                        progress.set_failure(get_failure_from_error(&e));
                        progress.write_increment(1);
