...
function Invoke-DscCacheRefresh {
    ...
    elseif ('Windows' -eq $module) {
        $DscResources = psDscAdapter\Get-DscResource | Where-Object { $_.modulename -eq $null -and $_.parentpath -like "$env:SYSTEMROOT\System32\Configuration\*" }
    }
    else {
    ...
    foreach ($dscResource in $DscResources) {
        ...
        $binaryBuiltInModulePaths = @(
            "$env:SYSTEMROOT\system32\Configuration\Schema\MSFT_FileDirectoryConfiguration"
            "$env:SYSTEMROOT\system32\Configuration\BaseRegistration"
        )
