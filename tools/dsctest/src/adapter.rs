...
pub fn adapt(resource_type: &str, input: &str, operation: &AdapterOperation) -> Result<String, String> {
    match operation {
        ...
        AdapterOperation::Get => match resource_type {
            "Adapted/One" => {
        ...
        AdapterOperation::Set | AdapterOperation::Test => match resource_type {
            "Adapted/One" => {
                let adapted_one: AdaptedOne =
                    serde_json::from_str(input).map_err(|e| format!("Failed to parse input for Adapted/One: {e}"))?;
                Ok(serde_json::to_string(&adapted_one).unwrap())
            }
            "Adapted/Two" => {
                let adapted_two: AdaptedTwo =
                    serde_json::from_str(input).map_err(|e| format!("Failed to parse input for Adapted/Two: {e}"))?;
                Ok(serde_json::to_string(&adapted_two).unwrap())
            }
            _ => Err(format!("Unknown resource type: {resource_type}")),
        },
        AdapterOperation::Export => match resource_type {
            "Adapted/One" => {
                let adapted_one = AdaptedOne {
        ...
        AdapterOperation::Validate => Ok("{\"valid\": true}".to_string()),
    }
}
