...
function Invoke-Script {
    ...
    try {
        ...
        Write-DscTrace -Level Error -Message $_
        exit 1
    }
    finally {
        $ps.Dispose()
...
function FindAndParseResourceDefinitions {
    ...
    Write-DscTrace -Level Trace -Message "Loading resources from file '$filePath'"
    #TODO: Ensure embedded instances in properties are working correctly
    [System.Management.Automation.Language.Token[]] $tokens = $null
    [System.Management.Automation.Language.ParseError[]] $errors = $null
    $ast = [System.Management.Automation.Language.Parser]::ParseFile($filePath, [ref]$tokens, [ref]$errors)
    foreach ($e in $errors) {
        Write-DscTrace -Level Error -Message ($e | Out-String)
    }
...
function LoadPowerShellClassResourcesFromModule {
    ...
    Write-DscTrace -Level Trace -Message "Loading resources from module '$($moduleInfo.Path)'"
    ...
    if ($moduleInfo.RootModule) {
        if (".psm1", ".ps1" -notcontains ([System.IO.Path]::GetExtension($moduleInfo.RootModule)) -and
            ...
            Write-DscTrace -Level Trace -Message "RootModule is neither psm1 nor ps1 '$($moduleInfo.RootModule)'"
...
function Invoke-DscCacheRefresh {
    ...
    if (Test-Path $cacheFilePath) {
        ...
        $cache = Get-Content -Raw $cacheFilePath | ConvertFrom-Json -Depth 10
        ...
        if ($cache.CacheSchemaVersion -ne $script:CurrentCacheSchemaVersion) {
            ...
            if ($null -eq $dscResourceCacheEntries -or $dscResourceCacheEntries.Keys.Count -eq 0) {
                # if there is nothing in the cache file - refresh cache
                $refreshCache = $true

                Write-DscTrace -Level Debug -Message "Filtered DscResourceCache cache is empty"
            }
            else {
                Write-DscTrace -Level Debug -Message "Checking cache for stale entries"

                foreach ($cacheEntry in $dscResourceCacheEntries.Values) {
                    ...
                    $cacheEntry.LastWriteTimes.PSObject.Properties | ForEach-Object {
                        ...
                        if (Test-Path $_.Name) {
                            ...
                            if (-not ($file_LastWriteTime.Equals($cache_LastWriteTime))) {
                                Write-DscTrace -Level Debug -Message "Detected stale cache entry '$($_.Name)'"
                                $refreshCache = $true
                                break
                            }
                        }
                        else {
                            Write-DscTrace -Level Debug -Message "Detected non-existent cache entry '$($_.Name)'"
                            $refreshCache = $true
                            break
                        }
                    }
                ...
                if (-not $refreshCache) {
                    ...
                    Write-DscTrace -Level Debug -Message "PSModulePath diff '$diff'"
        ...
        Write-DscTrace -Level Debug -Message "Cache file not found '$cacheFilePath'"
        $refreshCache = $true
    }
    ...
    if ($refreshCache) {
            ...
            foreach ($mod in $modules) {
                if (-not ($processedModuleNames.ContainsKey($mod.Name))) {
                    ...
                    if ($selectedMod.Count -gt 1) {
                        Write-DscTrace -Level Trace -Message "Found $($selectedMod.Count) modules with name '$($mod.Name)'"
        ...
        Write-DscTrace -Level Debug -Message "Saving Get-DscResource cache to '$cacheFilePath'"
        $jsonCache = $cache | ConvertTo-Json -Depth 90 -Compress
        ...
        switch ([dscResourceType]$cachedDscResourceInfo.ImplementationDetail) {
            ...
            'ClassBased' {
                try {
                    ...
                    Write-DscTrace -Level Trace -Message ($ValidProperties | ConvertTo-Json -Compress)
                            ...
                            if ($_.Value -is [System.Management.Automation.PSCustomObject]) {
                                if ($validateProperty -and $validateProperty.PropertyType -in @('PSCredential', 'System.Management.Automation.PSCredential')) {
                                    if (-not $_.Value.Username -or -not $_.Value.Password) {
                                        Write-DscTrace -Level Error -Message "Credential object '$($_.Name)' requires both 'username' and 'password' properties"
                                        exit 1
                                    }
                    ...
                    switch ($Operation) {
                        ...
                        'Export' {
                            ...
                            if ($null -eq $method) {
                                Write-DscTrace -Level Error -Message "Export method not implemented by resource '$($DesiredState.Type)'"
                                exit 1
                            }
                            $resultArray = @()
                    ...
                    Write-DscTrace -Level Error -Message "Exception: $($_ | Out-String)"
                    exit 1
                }
            }
            Default {
                Write-DscTrace -Level Error -Message "Resource ImplementationDetail not supported: $($cachedDscResourceInfo.ImplementationDetail)"
                exit 1
            }
        }

        Write-DscTrace -Level Trace -Message "Output: $($addToActualState | ConvertTo-Json -Depth 10 -Compress)"
        return $addToActualState
    }
    else {
        $dsJSON = $DesiredState | ConvertTo-Json -Depth 10
        Write-DscTrace -Level Error -Message "Can not find type '$($DesiredState.type)' for resource '$dsJSON'. Please ensure that Get-DscResource returns this resource type."
        exit 1
    }
}
...
class dscResourceCache {
    ...
    [System.Collections.Hashtable] $ResourceCache
}
