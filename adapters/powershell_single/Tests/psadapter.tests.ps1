...
Describe 'PowerShell single adapter resource tests' {
    ...
    BeforeAll {
        ...
        if ($IsLinux -or $IsMacOS) {
            $cacheFilePath = Join-Path $env:HOME ".dsc" "PSAdapterCache_v4.json"
        }
        else {
            $cacheFilePath = Join-Path $env:LocalAppData "dsc" "PSAdapterCache_v4.json"
        }
    }
    ...
    It 'Discovery includes class-based resources' {
        ...
        $r = dsc resource list '*' -a Microsoft.Adapter/PowerShell
        $LASTEXITCODE | Should -Be 0
        $resources = $r | ConvertFrom-Json
        ($resources | Where-Object { $_.Type -eq 'PSTestClassResource/PSTestClassResource' }).Count | Should -Be 1
        ($resources | Where-Object -Property type -EQ 'PSTestClassResource/PSTestClassResource').capabilities | Should -BeIn @('get', 'set', 'test', 'export')
        ($resources | Where-Object -Property type -EQ 'PSTestClassResource/PSNoExport').capabilities | Should -BeIn @('get', 'set', 'test')
    }

    It 'Get works on class-based resource' {

        $r = "{'Name':'PSTestClassResource1'}" | dsc resource get -r 'PSTestClassResource/PSTestClassResource' -f -
        $LASTEXITCODE | Should -Be 0
    ...
    It 'Test works on class-based resource' {
        ...
        $r = "{'Name':'PSTestClassResource1','Prop1':'ValueForProp1'}" | dsc resource test -r 'PSTestClassResource/PSTestClassResource' -f -
        $LASTEXITCODE | Should -Be 0
        $res = $r | ConvertFrom-Json
    ...
    It 'Set works on class-based resource' {
        ...
        $r = "{'Name':'PSTestClassResource1','Prop1':'ValueForProp1'}" | dsc resource set -r 'PSTestClassResource/PSTestClassResource' -f -
        $LASTEXITCODE | Should -Be 0
        $res = $r | ConvertFrom-Json
    ...
    It 'Export works on PS class-based resource' -Pending {
        ...
        $r = dsc resource export -r PSTestClassResource/PSTestClassResource
        $LASTEXITCODE | Should -Be 0
        $res = $r | ConvertFrom-Json
    ...
    It 'Get --all works on PS class-based resource' -Pending {
        ...
        $r = dsc resource get --all -r PSTestClassResource/PSTestClassResource 2>$null
        $LASTEXITCODE | Should -Be 0
        $res = $r | ConvertFrom-Json
    ...
    It 'Verify that ClearCache works in PSAdapter' {
        # generate the cache
        $null = dsc resource list '*' -a Microsoft.Adapter/PowerShell
        # call the ClearCache operation
        $scriptPath = Join-Path $PSScriptRoot '..' 'psadapter.ps1'
        $null = & $scriptPath -Operation ClearCache
        # verify that PSAdapter does not find the cache
        dsc -l debug resource list '*' -a Microsoft.Adapter/PowerShell 2> $TestDrive/tracing.txt
        $LASTEXITCODE | Should -Be 0
    ...
    It 'Verify that a new PS Cache version results in cache rebuid' {
        ...
        $cacheFilePath = if ($IsWindows) {
            # PS 6+ on Windows
            Join-Path $env:LocalAppData "dsc\PSAdapterCache_v4.json"
        }
        else {
            # either WinPS or PS 6+ on Linux/Mac
            if ($PSVersionTable.PSVersion.Major -le 5) {
                Join-Path $env:LocalAppData "dsc\WindowsPSAdapterCache_v4.json"
            }
            else {
                Join-Path $env:HOME ".dsc" "PSAdapterCache_v4.json"
            }
        }
    ...
    It 'Verify that removing a module results in cache rebuid' {
        ...
        Copy-Item -Recurse -Force -Path "$PSScriptRoot/PSTestClassResource" -Destination $TestDrive
        Copy-Item -Recurse -Force -Path "$PSScriptRoot/PSTestClassResource" -Destination "$PSScriptRoot/Backup/PSTestClassResource"
        Remove-Item -Recurse -Force -Path "$PSScriptRoot/PSTestClassResource"
        ...
        try {
            ...
            Remove-Item -Recurse -Force -Path "$TestDrive/PSTestClassResource"
            # verify that cache rebuid happened
            $null = dsc -l trace resource list '*' -a Microsoft.Adapter/PowerShell 2> $TestDrive/tracing.txt
            ...
            Copy-Item -Recurse -Force -Path "$PSScriptRoot/Backup/PSTestClassResource" -Destination "$PSScriptRoot"
    ...
    It 'Verify inheritance works in class-based resources' {
        ...
        $LASTEXITCODE | Should -Be 0
        $resources = $r | ConvertFrom-Json
        $t = $resources | ? { $_.Type -eq 'PSTestClassResource/PSTestClassResource' }
        $t.properties | Should -Contain "BaseProperty"
    }

    It 'Verify highest module version is loaded' {

        $srcPath = Join-Path $PSScriptRoot 'PSTestClassResource'
        $pathRoot1 = Join-Path $TestDrive 'A'
        $pathRoot2 = Join-Path $TestDrive 'B'
        $path1 = Join-Path $pathRoot1 'PSTestClassResource' '1.0'
        $path2 = Join-Path $pathRoot1 'PSTestClassResource' '1.1'
        $path3 = Join-Path $pathRoot2 'PSTestClassResource' '2.0'
        $path4 = Join-Path $pathRoot2 'PSTestClassResource' '2.0.1'
        ...
        try {
            ...
            $LASTEXITCODE | Should -Be 0
            $resources = $r | ConvertFrom-Json
            $r = @($resources | ? { $_.Type -eq 'PSTestClassResource/PSTestClassResource' })
            $r.Count | Should -Be 1
    ...
    It 'Verify that there are no cache rebuilds for several sequential executions' {
        ...
        dsc -l trace resource list -a Microsoft.Adapter/PowerShell 2> $TestDrive/tracing.txt
        "$TestDrive/tracing.txt" | Should -FileContentMatchExactly 'Constructing Get-DscResource cache'
    ...
    It 'Specifying version works' {
        $out = dsc resource get -r PSTestClassResource/PSTestClassResource --version 0.0.1 | ConvertFrom-Json
        $LASTEXITCODE | Should -Be 0
        $out.actualState.Ensure | Should -BeExactly 'Present'
    }

    It 'Specifying a non-existent version returns an error' {
        $null = dsc resource get -r PSTestClassResource/PSTestClassResource --version 0.0.2 2> $TestDrive/error.log
        $LASTEXITCODE | Should -Be 7
        Get-Content -Path $TestDrive/error.log | Should -Match 'Resource not found: PSTestClassResource/PSTestClassResource 0.0.2'
    }

    It 'Can process SecureString property' {
        $r = '{"Name":"PSTestClassResource1","SecureStringProp":"MySecretValue"}' | dsc resource get -r 'PSTestClassResource/PSTestClassResource' -f -
        $LASTEXITCODE | Should -Be 0
        $res = $r | ConvertFrom-Json
