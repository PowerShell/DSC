...
The output from the `dsc config test` command includes the desired and actual state of every
resource instance in the configuration, whether each instance is in the desired state, and the
list of properties that aren't in the desired state for each instance.
...
more information, see [dsc resource test result schema reference][13].
