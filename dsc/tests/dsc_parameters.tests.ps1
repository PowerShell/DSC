...
"@
    ...
    It 'Parameters defaults cannot use parameters() function' {
        $config_yaml = @"
            `$schema: https://aka.ms/dsc/schemas/v3/bundled/config/document.json
            parameters:
              basePrefix:
...
"@
        ...
        $null = $config_yaml | dsc config -p $params_json get -f - 2>$TestDrive/error.log
        $errorLog = Get-Content -Path $TestDrive/error.log -Raw
        $LASTEXITCODE | Should -Be 4 -Because $errorLog
        $errorLog | Should -BeLike "*The 'parameters()' function is not available within parameters defaults*"
    }
