...
Describe 'Tests for security features' {
    It 'Unsigned config file gives warning' -Skip:(!$IsWindows) {
        ...
        (Get-Content $TestDrive/error.log -Raw) | Should -Match "WARN File '.*?\\osinfo_parameters.dsc.yaml' is not signed.*?"
    }

    It 'Unsigned resource manifest gives warning' -Skip:(!$IsWindows) {
        $null = dsc resource get -r Microsoft/OSInfo 2>$TestDrive/error.log
        $LASTEXITCODE | Should -Be 0
        (Get-Content $TestDrive/error.log -Raw) | Should -Match "WARN File '.*?\\osinfo.dsc.resource.json' is not signed.*?"
    }

    It 'Unsigned resource executable gives warning' -Skip:(!$IsWindows) {
        $null = dsc resource get -r Microsoft/OSInfo 2>$TestDrive/error.log
        $LASTEXITCODE | Should -Be 0
        (Get-Content $TestDrive/error.log -Raw) | Should -Match "WARN File '.*?\\osinfo.exe' is not signed.*?"
    }
}
