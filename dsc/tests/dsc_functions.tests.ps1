...
#   }
  ...
  It 'json() works: <expression>' -TestCases @(
    @{ expression = "[json('{""name"":""John"",""age"":30}').name]"; expected = 'John' }
    @{ expression = "[json('{""name"":""John"",""age"":30}').age]"; expected = 30 }
    @{ expression = "[json('[1,2,3]')[0]]"; expected = 1 }
    @{ expression = "[json('[1,2,3]')[2]]"; expected = 3 }
    @{ expression = "[length(json('[1,2,3,4,5]'))]"; expected = 5 }
    @{ expression = "[json('""hello""')]"; expected = 'hello' }
    @{ expression = "[json('42')]"; expected = 42 }
    @{ expression = "[json('true')]"; expected = $true }
    @{ expression = "[json('false')]"; expected = $false }
    @{ expression = "[json('null')]"; expected = $null }
    @{ expression = "[json('{""users"":[{""name"":""Alice""},{""name"":""Bob""}]}').users[0].name]"; expected = 'Alice' }
    @{ expression = "[json('{""users"":[{""name"":""Alice""},{""name"":""Bob""}]}').users[1].name]"; expected = 'Bob' }
    @{ expression = "[json('  { ""key"" : ""value"" }  ').key]"; expected = 'value' }
    @{ expression = "[length(json('{""a"":1,""b"":2,""c"":3}'))]"; expected = 3 }
    @{ expression = "[json('{""nested"":{""value"":123}}').nested.value]"; expected = 123 }
  ) {
    param($expression, $expected)
