    ...
    pub fn secret(&self, name: &str, vault: Option<&str>) -> Result<Option<String>, DscError> {
        if self.capabilities.contains(&Capability::Secret) {
            debug!(
                "{}",
                t!(
                    "extensions.dscextension.retrievingSecretFromExtension",
                    name = name,
            ...
            if stdout.is_empty() {
                debug!(
                    "{}",
                    t!(
                        "extensions.dscextension.extensionReturnedNoSecret",
                        extension = self.type_name
                    )
                );
                Ok(None)
                ...
                let secret = if stdout.lines().count() > 1 {
                    return Err(DscError::NotSupported(
                        t!(
                            "extensions.dscextension.secretMultipleLinesReturned",
                            extension = self.type_name
                        )
                        .to_string(),
                    ));
                    ...
                    debug!(
                        "{}",
                        t!(
                            "extensions.dscextension.extensionReturnedSecret",
                            extension = self.type_name
                        )
                    );
