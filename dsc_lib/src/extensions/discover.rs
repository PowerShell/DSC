...
use crate::{
    discovery::command_discovery::{load_manifest, ImportedManifest},
    dscerror::DscError,
    dscresources::{
        command_resource::{invoke_command, process_args},
    ...
    pub fn discover(&self) -> Result<Vec<DscResource>, DscError> {
        ...
        if self.capabilities.contains(&Capability::Discover) {
            ...
            if stdout.is_empty() {
                info!(
                    "{}",
                    t!("extensions.dscextension.discoverNoResults", extension = self.type_name)
                );
            } else {
                for line in stdout.lines() {
                    trace!(
                        "{}",
                        t!(
                            "extensions.dscextension.extensionReturned",
                    ...
                    if !Path::new(&discover_result.manifest_path).is_absolute() {
                        return Err(DscError::Extension(
                            t!(
                                "extensions.dscextension.discoverNotAbsolutePath",
                                extension = self.type_name.clone(),
                                path = discover_result.manifest_path.clone()
                            )
                            .to_string(),
                        ));
                    }
