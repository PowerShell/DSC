...
use super::{
    ...
    invoke_result::{
        get_in_desired_state, ExportResult, GetResult, ResolveResult, ResourceGetResponse, ResourceSetResponse,
        ResourceTestResponse, SetResult, TestResult, ValidateResult,
    },
    resource_manifest::{ArgKind, InputKind, Kind, ResourceManifest, ReturnKind, SchemaKind},
};
use crate::configure::{
    config_doc::ExecutionKind,
    config_result::{ResourceGetResult, ResourceTestResult},
};
...
pub fn invoke_get(
...
) -> Result<GetResult, DscError> {
    debug!(
        "{}",
        t!(
            "dscresources.commandResource.invokeGet",
    ...
    info!(
        "{}",
        t!(
            "dscresources.commandResource.invokeGetUsing",
            resource = &resource.resource_type,
            executable = &get.executable
        )
    );
    let (_exit_code, stdout, stderr) = invoke_command(
        &get.executable,
        args,
        command_input.stdin.as_deref(),
        Some(cwd),
        command_input.env,
        resource.exit_codes.as_ref(),
    )?;
    if resource.kind == Some(Kind::Resource) {
        debug!(
            "{}",
            t!(
                "dscresources.commandResource.verifyOutputUsing",
                resource = &resource.resource_type,
                executable = &get.executable
            )
        );
    ...
    let result: GetResult = if let Ok(group_response) = serde_json::from_str::<Vec<ResourceGetResult>>(&stdout) {
        trace!(
            "{}",
            t!("dscresources.commandResource.groupGetResponse", response = &group_response : {:?})
        );
        ...
        let result: Value = match serde_json::from_str(&stdout) {
            ...
            Err(err) => {
                return Err(DscError::Operation(
                    t!(
                        "dscresources.commandResource.failedParseJson",
                        executable = &get.executable,
                        stdout = stdout,
                        stderr = stderr,
                        err = err
        ...
        GetResult::Resource(ResourceGetResponse { actual_state: result })
    };
...
pub fn invoke_set(
    ...
    target_resource: Option<&str>,
) -> Result<SetResult, DscError> {
    debug!(
        "{}",
        t!(
            "dscresources.commandResource.invokeSet",
            resource = &resource.resource_type
        )
    );
    ...
    if !skip_test && set.pre_test != Some(true) {
        info!(
            "{}",
            t!(
                "dscresources.commandResource.noPretest",
                resource = &resource.resource_type
            )
        );
        ...
        let (in_desired_state, actual_state) = match &test_result {
            ...
            TestResult::Resource(response) => (response.in_desired_state, response.actual_state.clone()),
        };
    ...
    if is_synthetic_what_if {
        return Err(DscError::NotImplemented(
            t!("dscresources.commandResource.syntheticWhatIf").to_string(),
        ));
    }
    ...
    info!(
        "{}",
        t!(
            "dscresources.commandResource.setGetCurrent",
            resource = &resource.resource_type,
            executable = &get.executable
        )
    );
    let (exit_code, stdout, stderr) = invoke_command(
        &get.executable,
        args,
        command_input.stdin.as_deref(),
        Some(cwd),
    ...
    if resource.kind == Some(Kind::Resource) {
        debug!(
            "{}",
            t!(
                "dscresources.commandResource.setVerifyGet",
                resource = &resource.resource_type,
                executable = &get.executable
            )
        );
    ...
    let mut pre_state = if pre_state_value.is_object() {
        ...
        if resource.kind == Some(Kind::Adapter)
            && pre_state_map.contains_key("result")
            && !pre_state_map.contains_key("resources")
        {
    ...
    info!(
        "Invoking {} '{}' using '{}'",
        operation_type, &resource.resource_type, &set.executable
    );
    let (exit_code, stdout, stderr) = invoke_command(
        &set.executable,
        args,
    ...
    match set.returns {
        Some(ReturnKind::State) => {
            if resource.kind == Some(Kind::Resource) {
                debug!(
                    "{}",
                    t!(
                        "dscresources.commandResource.setVerifyOutput",
        ...
        Some(ReturnKind::StateAndDiff) => {
            ...
            let Some(actual_line) = lines.next() else {
                return Err(DscError::Command(
                    resource.resource_type.clone(),
                    exit_code,
                    t!("dscresources.commandResource.setUnexpectedOutput").to_string(),
                ));
            };
            ...
            let Some(diff_line) = lines.next() else {
                return Err(DscError::Command(
                    resource.resource_type.clone(),
                    exit_code,
                    t!("dscresources.commandResource.setUnexpectedDiff").to_string(),
                ));
            };
        ...
        None => {
            ...
            let actual_state = match get_result {
                ...
                GetResult::Resource(response) => response.actual_state,
            };
...
pub fn invoke_test(
    ...
    expected: &str,
    target_resource: Option<&str>,
) -> Result<TestResult, DscError> {
    debug!(
        "{}",
        t!(
            "dscresources.commandResource.invokeTest",
            resource = &resource.resource_type
        )
    );
    let Some(test) = &resource.test else {
        info!(
            "{}",
            t!(
                "dscresources.commandResource.testSyntheticTest",
                resource = &resource.resource_type
            )
        );
    ...
    info!(
        "{}",
        t!(
            "dscresources.commandResource.invokeTestUsing",
            resource = &resource.resource_type,
    ...
    if resource.kind == Some(Kind::Resource) {
        debug!(
            "{}",
            t!(
                "dscresources.commandResource.testVerifyOutput",
                resource = &resource.resource_type,
                executable = &test.executable
            )
        );
        ...
        Some(ReturnKind::StateAndDiff) => {
            ...
            let Some(actual_value) = lines.next() else {
                return Err(DscError::Command(
                    resource.resource_type.clone(),
                    exit_code,
                    t!("dscresources.commandResource.testNoActualState").to_string(),
                ));
            };
            ...
            let Some(diff_properties) = lines.next() else {
                return Err(DscError::Command(
                    resource.resource_type.clone(),
                    exit_code,
                    t!("dscresources.commandResource.testNoDiff").to_string(),
                ));
            };
...
fn get_desired_state(actual: &Value) -> Result<Option<bool>, DscError> {
    ...
    if let Some(in_desired_state_value) = actual.get("_inDesiredState") {
        if let Some(desired_state) = in_desired_state_value.as_bool() {
            ...
            return Err(DscError::Operation(
                t!("dscresources.commandResource.inDesiredStateNotBool").to_string(),
            ));
        }
    }
    Ok(in_desired_state)
...
pub fn invoke_delete(
    ...
    info!(
        "{}",
        t!(
            "dscresources.commandResource.invokeDeleteUsing",
            resource = resource_type,
            executable = &delete.executable
        )
    );
    let (_exit_code, _stdout, _stderr) = invoke_command(
        &delete.executable,
        args,
        command_input.stdin.as_deref(),
        Some(cwd),
...
///
/// Error is returned if the underlying command returns a non-zero exit code.
pub fn invoke_validate(
    resource: &ResourceManifest,
    cwd: &str,
    config: &str,
    target_resource: Option<&str>,
) -> Result<ValidateResult, DscError> {
    trace!(
        "{}",
        t!(
            "dscresources.commandResource.invokeValidateConfig",
            resource = &resource.resource_type,
            config = &config
        )
    );
    ...
    info!(
        "{}",
        t!(
            "dscresources.commandResource.invokeValidateUsing",
            resource = resource_type,
    ...
    let Some(export) = resource.export.as_ref() else {
        ...
        if resource.get.is_some() {
            info!(
                "{}",
                t!(
                    "dscresources.commandResource.exportNotSupportedUsingGet",
                    resource = &resource.resource_type
                )
            );
        ...
        return Err(DscError::Operation(
            t!(
                "dscresources.commandResource.exportNotSupported",
                resource = &resource.resource_type
            )
            .to_string(),
        ));
    ...
    for line in stdout.lines() {
        ...
        if resource.kind == Some(Kind::Resource) {
            debug!(
                "{}",
                t!(
                    "dscresources.commandResource.exportVerifyOutput",
                    resource = &resource.resource_type,
                    executable = &export.executable
                )
            );
...
pub fn invoke_resolve(resource: &ResourceManifest, cwd: &str, input: &str) -> Result<ResolveResult, DscError> {
    let Some(resolve) = &resource.resolve else {
        return Err(DscError::Operation(
            t!(
                "dscresources.commandResource.resolveNotSupported",
                resource = &resource.resource_type
            )
            .to_string(),
        ));
    ...
    info!(
        "{}",
        t!(
            "dscresources.commandResource.invokeResolveUsing",
            resource = &resource.resource_type,
...
async fn run_process_async(
    ...
    exit_codes: Option<&HashMap<i32, String>>,
) -> Result<(i32, String, String), DscError> {
    ...
    let Some(stdout) = child.stdout.take() else {
        return Err(DscError::CommandOperation(
            t!("dscresources.commandResource.processChildStdout").to_string(),
            executable.to_string(),
        ));
    };
    let Some(stderr) = child.stderr.take() else {
        return Err(DscError::CommandOperation(
            t!("dscresources.commandResource.processChildStderr").to_string(),
            executable.to_string(),
        ));
    };
    ...
    if let Some(input) = input {
        ...
        let Some(mut stdin) = child.stdin.take() else {
            return Err(DscError::CommandOperation(
                t!("dscresources.commandResource.processChildStdin").to_string(),
                executable.to_string(),
            ));
        };
        if stdin.write_all(input.as_bytes()).await.is_err() {
            return Err(DscError::CommandOperation(
                t!("dscresources.commandResource.processWriteStdin").to_string(),
                executable.to_string(),
            ));
        }
        drop(stdin);
    ...
    let Some(child_id) = child.id() else {
        return Err(DscError::CommandOperation(
            t!("dscresources.commandResource.processChildId").to_string(),
            executable.to_string(),
        ));
    };
    ...
    if let Some(code) = exit_code {
        debug!(
            "{}",
            t!(
                "dscresources.commandResource.processChildExit",
        ...
        debug!(
            "{}",
            t!(
                "dscresources.commandResource.processChildTerminated",
                executable = executable,
                id = child_id
            )
        );
        Err(DscError::CommandOperation(
            t!("dscresources.commandResource.processTerminated").to_string(),
            executable.to_string(),
        ))
    }
}
...
pub fn invoke_command(
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async {
            trace!(
                "{}",
                t!("dscresources.commandResource.commandInvoke", executable = executable, args = args : {:?})
            );
            if let Some(cwd) = cwd {
            ...
            match run_process_async(executable, args, input, cwd, env, exit_codes).await {
                Ok((code, stdout, stderr)) => Ok((code, stdout, stderr)),
                Err(err) => {
                    error!(
                        "{}",
                        t!(
                            "dscresources.commandResource.runProcessError",
                            executable = executable,
                            error = err
                        )
                    );
...
fn verify_json(resource: &ResourceManifest, cwd: &str, json: &str) -> Result<(), DscError> {
    debug!(
        "{}",
        t!(
            "dscresources.commandResource.verifyJson",
            resource = resource.resource_type
        )
    );
    ...
    if resource.validate.is_some() {
        ...
        return Err(DscError::Validation(
            t!("dscresources.commandResource.resourceInvalidJson").to_string(),
        ));
    }
    ...
    if let Value::Object(obj) = json {
        for (key, value) in obj {
            match value {
                ...
                Value::Array(a) => {
                    ...
                    for v in a {
                        match v {
                            ...
                            _ => {
                                return Err(DscError::Operation(
                                    t!("dscresources.commandResource.invalidArrayKey", key = key).to_string(),
                                ));
                            }
                        }
                    }
                ...
                Value::Object(_) => {
                    return Err(DscError::Operation(
                        t!("dscresources.commandResource.invalidKey", key = key).to_string(),
                    ));
                }
            }
        }
...
pub fn log_stderr_line<'a>(process_id: &u32, trace_line: &'a str) -> &'a str {
    if !trace_line.is_empty() {
        if let Ok(trace_object) = serde_json::from_str::<Trace>(trace_line) {
            ...
            let target = if let Some(t) = trace_object.target.as_deref() {
                t
            } else {
                include_target = false;
                ""
            };
            let line_number = if let Some(l) = trace_object.line_number {
                l
            } else {
                include_target = false;
                0
            ...
            let trace_message = if include_target {
                format!(
                    "PID {process_id}: {target}: {line_number}: {}",
                    trace_object.fields.message
                )
            } else {
