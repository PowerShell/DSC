...
impl ResourceDiscovery for CommandDiscovery {
    ...
    fn discover_adapted_resources(&mut self, name_filter: &str, adapter_filter: &str, progress_format: &Option<OutputFormat>) -> Result<(), DscError> {
        ...
        let mut pb_span = DscProgressBar::new(<std::option::Option<OutputFormat> as Clone>::clone(&progress_format).is_some_and(|v| (v == OutputFormat::Json) || (v == OutputFormat::PrettyJson)));
        pb_span.pb_set_style(&ProgressStyle::with_template(
            "{spinner:.green} [{elapsed_precise:.cyan}] [{bar:40.cyan/blue}] {pos:>7}/{len:7} {msg:.yellow}"
        )?);
        ...
        for (adapter_name, adapters) in &self.adapters {
            ...
            for adapter in adapters {
                ...
                let mut pb_adapter_span = DscProgressBar::new(<std::option::Option<OutputFormat> as Clone>::clone(&progress_format).is_some_and(|v| (v == OutputFormat::Json) || (v == OutputFormat::PrettyJson)));
