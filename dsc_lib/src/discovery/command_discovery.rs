...
use crate::extensions::dscextension::{self, Capability as ExtensionCapability, DscExtension};
...
use crate::{locked_clone, locked_extend, locked_get, locked_is_empty};
...
static ADAPTED_RESOURCES: LazyLock<RwLock<BTreeMap<String, Vec<DscResource>>>> =
...
impl CommandDiscovery {
    ...
    pub fn new(progress_format: ProgressFormat) -> CommandDiscovery {
        CommandDiscovery { progress_format }
    }

    #[must_use]
    pub fn get_extensions(&self) -> BTreeMap<String, DscExtension> {
        locked_clone!(EXTENSIONS)
    }
    ...
    fn get_resource_path_setting() -> Result<ResourcePathSetting, DscError> {
        if let Ok(v) = get_setting("resourcePath") {
            ...
            if v.policy != serde_json::Value::Null {
                match serde_json::from_value::<ResourcePathSetting>(v.policy) {
                    ...
                    Err(e) => {
                        return Err(DscError::Setting(format!("{e}")));
                    }
                }
        ...
        Err(DscError::Setting(
            t!("discovery.commandDiscovery.couldNotReadSetting").to_string(),
        ))
    }
    ...
    fn get_resource_paths() -> Result<Vec<PathBuf>, DscError> {
                ...
                match env::var_os("PATH") {
                    Some(value) => {
                        trace!(
                            "{}",
                            t!(
                                "discovery.commandDiscovery.originalPath",
                    ...
                    None => {
                        return Err(DscError::Operation(
                            t!("discovery.commandDiscovery.failedGetEnvPath").to_string(),
                        ));
                    }
                }
            }
            ...
            if let Some(exe_home) = get_exe_path()?.parent() {
                ...
                if paths.contains(&exe_home_pb) {
                    trace!(
                        "{}",
                        t!(
                            "discovery.commandDiscovery.exeHomeAlreadyInPath",
                            path = exe_home.to_string_lossy()
                        )
                    );
                } else {
                    trace!(
                        "{}",
                        t!(
                            "discovery.commandDiscovery.addExeHomeToPath",
                            path = exe_home.to_string_lossy()
                        )
                    );
        ...
        if let Ok(final_resource_path) = env::join_paths(paths.clone()) {
            debug!(
                "{}",
                t!(
                    "discovery.commandDiscovery.usingResourcePath",
                    path = final_resource_path.to_string_lossy()
                )
            );
        }
...
impl ResourceDiscovery for CommandDiscovery {
    ...
    fn discover(&mut self, kind: &DiscoveryKind, filter: &str) -> Result<(), DscError> {
        info!(
            "{}",
            t!("discovery.commandDiscovery.discoverResources", kind = kind : {:?}, filter = filter)
        );
        ...
        let Ok(regex) = regex_builder.build() else {
            return Err(DscError::Operation(
                t!("discovery.commandDiscovery.invalidAdapterFilter").to_string(),
            ));
        };
                        ...
                        if path.is_file() {
                                        ...
                                        .any(|ext| file_name_lowercase.ends_with(ext)))
                            {
                                trace!(
                                    "{}",
                                    t!(
                                        "discovery.commandDiscovery.foundResourceManifest",
                                    ...
                                    ImportedManifest::Extension(extension) => {
                                        if regex.is_match(&extension.type_name) {
                                            trace!(
                                                "{}",
                                                t!(
                                                    "discovery.commandDiscovery.extensionFound",
                                                    extension = extension.type_name,
                                            ...
                                            if let Some(existing_extension) = extensions.get_mut(&extension.type_name) {
                                                let Ok(existing_version) = Version::parse(&existing_extension.version)
                                                else {
                                                    return Err(DscError::Operation(
                                                        t!(
                                                            "discovery.commandDiscovery.extensionInvalidVersion",
                                                            extension = existing_extension.type_name,
                                                            version = existing_extension.version
                                                        )
                                                        .to_string(),
                                                    ));
                                                };
                                            ...
                                            if let Some(ref manifest) = resource.manifest {
                                                ...
                                                if manifest.kind == Some(Kind::Adapter) {
                                                    trace!(
                                                        "{}",
                                                        t!(
                                                            "discovery.commandDiscovery.adapterFound",
                                                            adapter = resource.type_name,
                                                ...
                                                trace!(
                                                    "{}",
                                                    t!(
                                                        "discovery.commandDiscovery.resourceFound",
                ...
                for extension in locked_clone!(EXTENSIONS).values() {
                    if extension.capabilities.contains(&ExtensionCapability::Discover) {
                        debug!(
                            "{}",
                            t!(
                                "discovery.commandDiscovery.callingExtension",
                                extension = extension.type_name
                            )
                        );
                        let discovered_resources = match extension.discover() {
                            ...
                            Err(e) => {
                                warn!(
                                    "{}",
                                    t!(
                                        "discovery.commandDiscovery.extensionDiscoverFailed",
                                        extension = extension.type_name,
                                        error = e
                                    )
                                );
                        ...
                        debug!(
                            "{}",
                            t!(
                                "discovery.commandDiscovery.extensionFoundResources",
                                extension = extension.type_name,
                                count = discovered_resources.len()
                            )
                        );
                        for resource in discovered_resources {
                            if regex.is_match(&resource.type_name) {
                                trace!(
                                    "{}",
                                    t!(
                                        "discovery.commandDiscovery.extensionResourceFound",
                                        resource = resource.type_name
                                    )
                                );
                                insert_resource(&mut resources, &resource);
    ...
    fn discover_adapted_resources(&mut self, name_filter: &str, adapter_filter: &str) -> Result<(), DscError> {
        ...
        let Ok(regex) = regex_builder.build() else {
            return Err(DscError::Operation(
                "Could not build Regex filter for adapter name".to_string(),
            ));
        };
        ...
        let Ok(name_regex) = name_regex_builder.build() else {
            return Err(DscError::Operation(
                "Could not build Regex filter for resource name".to_string(),
            ));
        };
            ...
            for adapter in adapters {
                ...
                let manifest = if let Some(manifest) = &adapter.manifest {
                    if let Ok(manifest) = import_manifest(manifest.clone()) {
                        ...
                        return Err(DscError::Operation(format!(
                            "Failed to import manifest for '{}'",
                            adapter_name.clone()
                        )));
                    }
                } else {
                ...
                let (exit_code, stdout, stderr) = match invoke_command(
                    &list_command.executable,
                    list_command.args,
                    None,
                    Some(&adapter.directory),
                    None,
                    manifest.exit_codes.as_ref(),
                ) {
                    ...
                    match serde_json::from_str::<DscResource>(line) {
                        Result::Ok(resource) => {
                            if resource.require_adapter.is_none() {
                                warn!(
                                    "{}",
                                    DscError::MissingRequires(adapter_name.clone(), resource.type_name.clone())
                                        .to_string()
                                );
                                continue;
                            }
                ...
                debug!(
                    "Adapter '{}' listed {} resources",
                    adapter_name, adapter_resources_count
                );
            }
        }
        ...
        if *kind == DiscoveryKind::Resource {
            if adapter_name_filter.is_empty() {
                ...
                for (resource_name, resources_vec) in &locked_clone!(RESOURCES) {
                    resources.insert(
                        ...
                        resources_vec
                            .iter()
                            .map(|r| ImportedManifest::Resource(r.clone()))
                            .collect(),
                    );
                }
            ...
            for (extension_name, extension) in &locked_clone!(EXTENSIONS) {
                resources.insert(
                    ...
                    vec![ImportedManifest::Extension(extension.clone())],
                );
            }
        }
    ...
    fn find_resources(
        ...
        debug!(
            "{}",
            t!("discovery.commandDiscovery.searchingForResources", resources = required_resource_types : {:?})
        );
        ...
        debug!(
            "{}",
            t!(
                "discovery.commandDiscovery.foundNonAdapterResources",
                count = found_resources.len()
            )
        );
...
fn filter_resources(
    found_resources: &mut BTreeMap<String, Vec<DscResource>>,
    required_resources: &mut HashMap<DiscoveryFilter, bool>,
    resources: &[DscResource],
            ...
            if let Ok(resource_version) = Version::parse(&resource.version) {
                if let Ok(version_req) = VersionReq::parse(required_version) {
                    if version_req.matches(&resource_version) {
                        found_resources
                            .entry(filter.resource_type().to_string())
                            .or_default()
                            .push(resource.clone());
                        ...
                        debug!(
                            "{}",
                            t!(
                                "discovery.commandDiscovery.foundResourceWithVersion",
                                resource = resource.type_name,
                                version = resource.version
                            )
                        );
...
pub fn load_manifest(path: &Path) -> Result<ImportedManifest, DscError> {
    ...
    if path.extension() == Some(OsStr::new("json")) {
        ...
        let manifest = match serde_json::from_str::<ResourceManifest>(&contents) {
            ...
            Err(err) => {
                return Err(DscError::Manifest(
                    t!(
                        "discovery.commandDiscovery.invalidManifest",
    ...
    let import_extensions = if let Some(import) = &manifest.import {
        ...
        if import.file_extensions.is_empty() {
            warn!(
                "{}",
                t!(
                    "discovery.commandDiscovery.importExtensionsEmpty",
                    extension = manifest.r#type
                )
            );
            None
        } else {
...
fn verify_executable(resource: &str, operation: &str, executable: &str) {
    if which(executable).is_err() {
        info!(
            "{}",
            t!(
                "discovery.commandDiscovery.executableNotFound",
                resource = resource,
                operation = operation,
    ...
    for (resource_name, res_vec) in adapted_resources {
        if let Some(adapter_name) = &res_vec[0].require_adapter {
            ...
            debug!(
                "{}",
                t!(
                    "discovery.commandDiscovery.resourceMissingRequireAdapter",
...
fn save_adapted_resources_lookup_table(lookup_table: &HashMap<String, String>) {
    if let Ok(lookup_table_json) = serde_json::to_string(&lookup_table) {
        ...
        debug!(
            "Saving lookup table with {} items to {:?}",
            lookup_table.len(),
            file_path
        );
...
fn load_adapted_resources_lookup_table() -> HashMap<String, String> {
    ...
    debug!(
        "Read {} items into lookup table from {:?}",
        lookup_table.len(),
        file_path
    );
    lookup_table
...
fn get_lookup_table_file_path() -> String {
    ...
    let Ok(local_app_data_path) = std::env::var("LocalAppData") else {
        return String::new();
    };

    Path::new(&local_app_data_path)
        .join("dsc")
        .join("AdaptedResourcesLookupTable.json")
        .display()
        .to_string()
}
