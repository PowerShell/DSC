    ...
    fn invoke(&self, args: &[Value], _context: &Context) -> Result<Value, DscError> {
        ...
        if !args.len().is_multiple_of(2) {
            return Err(DscError::Parser(
                t!("functions.createObject.argsMustBePairs").to_string(),
            ));
        }
        ...
        for chunk in args.chunks(2) {
            ...
            if !key.is_string() {
                return Err(DscError::Parser(
                    t!("functions.createObject.keyMustBeString").to_string(),
                ));
            }
...
mod tests {
    ...
    fn simple_object() {
        ...
        let result = parser
            .parse_and_execute("[createObject('name', 'test')]", &Context::new())
            .unwrap();
    ...
    fn multiple_properties() {
        let mut parser = Statement::new().unwrap();
        let result = parser
            .parse_and_execute("[createObject('name', 'test', 'value', 42)]", &Context::new())
            .unwrap();
    ...
    fn nested_objects() {
        ...
        let result = parser
            .parse_and_execute(
                "[createObject('outer', createObject('inner', 'value'))]",
                &Context::new(),
            )
            .unwrap();
    ...
    fn with_arrays() {
        let mut parser = Statement::new().unwrap();
        let result = parser
            .parse_and_execute("[createObject('items', createArray('a', 'b', 'c'))]", &Context::new())
            .unwrap();
