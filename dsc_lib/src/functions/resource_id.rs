    ...
    fn invoke(&self, args: &[Value], _context: &Context) -> Result<Value, DscError> {
        ...
        if let Some(value) = resource_type.as_str() {
            ...
            if slash_count != 1 {
                return Err(DscError::Function(
                    ...
                    t!("functions.resourceId.incorrectTypeFormat").to_string(),
                ));
            }
            result.push_str(value);
        } else {
            return Err(DscError::Parser(
                t!("functions.resourceId.invalidFirstArgType").to_string(),
            ));
        }
        ...
        if let Some(value) = resource_name.as_str() {
            if value.contains('/') {
                return Err(DscError::Function(
                    "resourceId".to_string(),
                    t!("functions.resourceId.incorrectNameFormat").to_string(),
                ));
            }
            ...
            return Err(DscError::Parser(
                t!("functions.resourceId.invalidSecondArgType").to_string(),
            ));
        }
...
mod tests {
    ...
    fn strings() {
        ...
        let result = parser
            .parse_and_execute("[resourceId('a/b', 'c')]", &Context::new())
            .unwrap();
    ...
    fn strings_with_dots() {
        let mut parser = Statement::new().unwrap();
        let result = parser
            .parse_and_execute("[resourceId('a.b/c', 'd')]", &Context::new())
            .unwrap();
