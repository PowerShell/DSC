...
impl DscManager {
    ...
    pub fn list_available_resources(&mut self, type_name_filter: &str, adapter_name_filter: &str, progress_format: OutputFormat) -> Vec<DscResource> {
        self.discovery.list_available_resources(type_name_filter, adapter_name_filter, progress_format)
    }

    pub fn find_resources(&mut self, required_resource_types: &[String], progress_format: OutputFormat) {
